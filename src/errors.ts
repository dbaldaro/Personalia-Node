...
export class PersonaliaError extends Error {
...
export const PERSONALIA_ERRORS: ErrorMapping = {
  '101': {
 ...
 */
export function parseApiError(error: any): PersonaliaError {
  ...
  if (error.response) {
    ...
    if (responseData) {
      ...
      if (responseData.errorId) {
        ...
        if (errorId && PERSONALIA_ERRORS[errorId]) {
          const errorInfo = PERSONALIA_ERRORS[errorId];
